    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            ...
            for mu, sigma_inv in zip(self.mu, self.sigma_inv):
                diff = X[i, :] - mu
                dist = torch.mm(torch.mm(diff, sigma_inv), diff.t())
    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            ...
            for mu, sigma_inv, bias, common_part in zip(self.mu, self.sigma_inv, self.biases, self.common_part):
                calc = bias - 0.5*(common_part + self.feature_dim)*torch.log(1.0 + (1.0/common_part)*self.compute_distance(X[i, :], mu, sigma_inv))
                neg_distrances.append(calc)
            predicts_matrix.append(torch.cat(neg_distrances, dim=1))
